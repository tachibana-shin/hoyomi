// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, unused_import

// Clients
export 'client/client_client.dart';
// Data classes
export 'models/comic_list_watch_history.dart';
export 'models/list_watch_page_schema.dart';
export 'models/watch_page_schema.dart';
export 'models/set_watch_page_body_schema.dart';
export 'models/has_follow_response.dart';
export 'models/set_follow_response.dart';
export 'models/set_follow_body_schema.dart';
export 'models/eiga_list_watch_history.dart';
export 'models/list_watch_time_schema.dart';
export 'models/watch_time_schema.dart';
export 'models/set_watch_time_body_schema.dart';
export 'models/has_eiga_follow_response.dart';
export 'models/set_eiga_follow_response.dart';
export 'models/set_eiga_follow_body_schema.dart';
export 'models/post_api_comic_set_watch_page_response.dart';
export 'models/items.dart';
export 'models/get_api_comic_get_list_follow_response.dart';
export 'models/ignore.dart';
export 'models/post_api_eiga_set_watch_time_response.dart';
export 'models/items2.dart';
export 'models/get_api_eiga_get_list_follow_response.dart';
export 'models/ignore2.dart';
export 'models/data.dart';
export 'models/data2.dart';
export 'models/data3.dart';
export 'models/data4.dart';
export 'models/data5.dart';
export 'models/data6.dart';
export 'models/status.dart';
export 'models/set_watch_page_body_schema_status.dart';
export 'models/set_follow_body_schema_status.dart';
export 'models/set_watch_time_body_schema_status.dart';
export 'models/set_eiga_follow_body_schema_status.dart';
// Root client
export 'general_api_client.dart';
