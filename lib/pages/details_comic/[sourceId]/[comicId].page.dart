              ...
              child: Comments(
                getComments: ({page, parent}) {
                  return (_service as ComicAuthMixin).getComments!(
                      comicId: widget.comicId, page: page, parent: parent);
                },
  ...
  Widget _buildSuggest(MetaComic comic) {
    ...
    return HorizontalComicList(
      itemsFuture: _suggestFuture!.then((value) => value.items
          .map((comic) => ComicExtend(comic: comic, sourceId: _service.uid))
          .toList()),
...
class _ButtonLike extends StatefulWidget {
  ...
  const _ButtonLike(
      {required this.comicId, required this.comic, required this.service});
