          ...
          items: topWeek.data
              .map(
                (item) => Comic(
                  ...
                  lastUpdate: item.newestChapterCreatedAt != null
...
sealed class Author with _$Author {
  const factory Author({required String name}) = _Author;
...
sealed class Team with _$Team {
  const factory Team({required String name}) = _Team;
  factory Team.fromJson(Map<String, dynamic> json) => _$TeamFromJson(json);
}

@freezed
sealed class Title with _$Title {
  const factory Title({required String name}) = _Title;
...
sealed class ChaptersResponse with _$ChaptersResponse {
  const factory ChaptersResponse({required List<Chapter> data}) =
      _ChaptersResponse;
