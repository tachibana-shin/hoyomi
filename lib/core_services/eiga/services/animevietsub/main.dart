...
class AnimeVietsubService extends ABEigaService
  ...
  getDetails(String eigaId) async {
    ...
    final genres = $('.AAIco-adjust > strong')
        .contains("Thể loại:")
        .parent()
        .query("a")
  ...
  getEpisodes(String eigaId) async {
    ...
    final episodes = $('#list-server .list-episode .episode a').map((item) {
  ...
  @override
  getWatchHistory({required int page}) async {
    final userUid = await _getUidUser();

    final data =
        (await rpc('query_history', {
                  'user_uid': userUid,
                  'page': page,
                  'size': 30,
                })
